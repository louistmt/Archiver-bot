...
async function execute(interaction: ChatInputCommandInteraction) {
    ...
    if (matches === null) {
        await interaction.reply({ephemeral: true, content: multiline(
            "Invalid dice format. Valid examples:",
            "- '1d6': Rolls a single 6 die",
            "- '2d6': Rolls two 6 dice",
            "- '2d6+3': Rolls two 6 die and adds 3 to the result of each die"
        )})
        return
    }
...
const rollCmd: Command = {definition, execute}
